...
///
pub fn edit_task(frame: &mut Frame, size: Rect, state: &mut State) {
    ...
    let title_block = Block::default().borders(Borders::ALL).title("Edit Task");
...
fn render_multi_enum_dropdown(
    ...
    let items: Vec<ListItem> = visible_options
        ...
        .map(|eo| {
            ...
            ListItem::new(format!("{}{}", prefix, eo.name))
        })
        .collect();
